...
- Multipart binary fields are detected for top-level, nested-object, and array properties. Array items produce repeated multipart parts under the same field name, in input order.
...
- Streaming responses remain bounded finite tool results. `text/event-stream` is parsed into an ordered structured event log after completion; generic chunked transfer remains its final payload because Go intentionally hides chunk boundaries. Proxy and dynamic modes enforce a 16 MiB default response cap before transformation.
